...
class DocutilsRenderer(RendererProtocol):
    ...
    def render_front_matter(self, token: SyntaxTreeNode) -> None:
        ...
        metadata = ""
        # parse metadata
        for meta in ["layout", "date", "title", "tags", "udc"]:
            if data.get(meta):
                match meta:
                    case "tags":
                        # e.g. "['a','b']"
                        #   -> "'a' 'b'"
                        text = f'{data["tags"]}'[1:-1].split(",")
                        res=''
                        # e.g. 'a'
                        #   -> a
                        for t in text:
                            t = t.replace("'","").replace(" ","")
                            res += '<span class="md-tag" id="tag">' + t + '</span>'
                        metadata += res

                    case "udc":
                        text = f'{data["udc"]}'[1:-1].split(",")
                        res=''
                        for t in text:
                            t = t.replace("'","").replace(" ","")
                            res += '<span class="md-tag" id="udc">' + t + '</span>'
                        metadata += res
                    ...
                    case "layout":
                        res = '<span class="md-tag" id="layout">' + f"{data['layout']}" + '</span>'
                        metadata += res

                    case "date":
                        res = '<span class="md-tag" id="date">' + f"{data['date']}" + '</span>'
                        metadata += res

        metadata = '<nav class="md-tag">' + metadata + '</nav>'
        self.nested_render_text(metadata, 0)
